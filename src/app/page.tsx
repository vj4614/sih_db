...
export default function Home() {
  ...
  // Mode & animation state (newbie / researcher)
  ...
  const handleApplyFilters = () => {
    const targetFloat = mockFloats.find(
    ...
    if (targetFloat) {
      ...
      const mockRegionCenters: Record<string, LatLngExpression> = {
        "Indian Ocean": [0, 80],
  ...
  // Render content for the active tab — keeps both newbie and researcher flows
  const renderTabContent = () => {
    ...
    // Researcher mode (more advanced UI)
    switch (activeTab) {
      case "chat":
        return (
          <ChatTab
    ...
    <div className="flex h-screen bg-background text-foreground font-sans">
      ...
      >
        ...
        <div className="flex h-full">
          {/* SidePanel (left) — keep it compact on small screens */}
          <aside className="hidden md:block">
            <SidePanel
              activeTab={activeTab}
              onTabClick={(tab: string) => setActiveTab(tab as Tab)}
              tabs={TABS}
            />
          </aside>
