...
export default function ChatDataViewer({ dataType, onClose }) {
    ...
    return (
        <div className="relative h-full bg-card/80 backdrop-blur-lg rounded-2xl shadow-2xl shadow-primary/20 border border-white/10 dark:border-gray-800/20 p-6 sm:p-8 animate-fade-in flex flex-col">
            ...
            <div className="flex-1 flex flex-col items-center justify-center">
                <div className="w-full max-w-2xl bg-card rounded-lg shadow-md overflow-hidden animate-plot-appear">
                    <h3 className="text-xl font-bold p-4 pb-2 text-foreground text-center">{title}</h3>
                    <div className="overflow-x-auto p-4 max-h-[calc(100vh-250px)]">
                        <table className="min-w-full text-sm text-foreground rounded-lg">
                            <thead className="bg-muted/50 text-muted-foreground border-b dark:border-gray-700">
                                <tr className="text-left">
                                    {columns.map(col => (
                                        <th key={col.key} className="p-3 font-semibold">{col.name}</th>
                                    ))}
                                </tr>
                            </thead>
                            <tbody>
                                {dataToDisplay.map((row, index) => (
                                    <tr key={index} className="border-b dark:border-gray-700 hover:bg-muted/50 transition-colors">
                                        {columns.map(col => (
                                            <td key={`${index}-${col.key}`} className="p-3">
